...
// Récupération du trajet à modifier
$stmt = $pdo->prepare("
    SELECT t.*, 
    ...
    WHERE t.id = ? AND t.conducteur_id = ? AND t.statut = 'planifie'
");
$stmt->execute([$trajet_id, $_SESSION['user_id']]);
$trajet = $stmt->fetch();

if (!$trajet) {
    flash('danger', 'Trajet non trouvé ou non modifiable');
    redirect('mes_trajets.php');
}

// Récupération des wilayas et véhicules
$wilayas = $pdo->query("SELECT * FROM wilayas ORDER BY nom")->fetchAll();
$vehicules = $pdo->prepare("SELECT * FROM vehicules WHERE utilisateur_id = ? ORDER BY date_ajout DESC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        flash('success', 'Trajet modifié avec succès !');
        redirect('mes_trajets.php');
    }
}
?>
...
<head>
    ...
    <title>Modifier un trajet - <?= SITE_NAME ?></title>
            ...
            <div class="col-lg-8">
                <div class="card shadow">
                    <div class="card-header bg-primary text-white">
                        <h3 class="mb-0">Modifier le trajet</h3>
                    </div>
                    <div class="card-body">
                        <?php display_flash(); ?>
                        ...
                        <form method="POST">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="wilaya_depart_id" name="wilaya_depart_id" required>
                                            ...
                                            <?php foreach ($wilayas as $wilaya): ?>
                                                <option value="<?= $wilaya['id'] ?>" <?= ($wilaya['id'] == $trajet['wilaya_depart_id'] || (!empty($_POST['wilaya_depart_id']) && $_POST['wilaya_depart_id'] == $wilaya['id'])) ? 'selected' : '' ?>>
                                                    <?= $wilaya['nom'] ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="wilaya_arrivee_id" name="wilaya_arrivee_id" required>
                                            <option value="">Choisir une wilaya</option>
                                            <?php foreach ($wilayas as $wilaya): ?>
                                                <option value="<?= $wilaya['id'] ?>" <?= ($wilaya['id'] == $trajet['wilaya_arrivee_id'] || (!empty($_POST['wilaya_arrivee_id']) && $_POST['wilaya_arrivee_id'] == $wilaya['id'])) ? 'selected' : '' ?>>
                                                    <?= $wilaya['nom'] ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="lieu_depart" name="lieu_depart" required
                                            value="<?= !empty($_POST['lieu_depart']) ? htmlspecialchars($_POST['lieu_depart']) : htmlspecialchars($trajet['lieu_depart']) ?>">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="lieu_arrivee" name="lieu_arrivee" required
                                            value="<?= !empty($_POST['lieu_arrivee']) ? htmlspecialchars($_POST['lieu_arrivee']) : htmlspecialchars($trajet['lieu_arrivee']) ?>">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="datetime-local" class="form-control" id="date_depart" name="date_depart" required
                                            value="<?= !empty($_POST['date_depart']) ? htmlspecialchars($_POST['date_depart']) : date('Y-m-d\TH:i', strtotime($trajet['date_depart'])) ?>">
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control" id="places_disponibles" name="places_disponibles" min="1" required
                                            value="<?= !empty($_POST['places_disponibles']) ? htmlspecialchars($_POST['places_disponibles']) : htmlspecialchars($trajet['places_disponibles']) ?>">
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control" id="prix" name="prix" min="0" step="50" required
                                            value="<?= !empty($_POST['prix']) ? htmlspecialchars($_POST['prix']) : htmlspecialchars($trajet['prix']) ?>">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="vehicule_id" name="vehicule_id" required>
                                    ...
                                    <?php foreach ($vehicules as $vehicule): ?>
                                        <option value="<?= $vehicule['id'] ?>" <?= ($vehicule['id'] == $trajet['vehicule_id'] || (!empty($_POST['vehicule_id']) && $_POST['vehicule_id'] == $vehicule['id'])) ? 'selected' : '' ?>>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="description" name="description" rows="3"><?= !empty($_POST['description']) ? htmlspecialchars($_POST['description']) : htmlspecialchars($trajet['description']) ?></textarea>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="bagages_autorises" name="bagages_autorises" value="1" <?= ($trajet['bagages_autorises'] || !empty($_POST['bagages_autorises'])) ? 'checked' : '' ?>>
                                    <label class="form-check-label" for="bagages_autorises">Bagages autorisés</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="animaux_autorises" name="animaux_autorises" value="1" <?= ($trajet['animaux_autorises'] || !empty($_POST['animaux_autorises'])) ? 'checked' : '' ?>>
                                    <label class="form-check-label" for="animaux_autorises">Animaux autorisés</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="fumeur_autorise" name="fumeur_autorise" value="1" <?= ($trajet['fumeur_autorise'] || !empty($_POST['fumeur_autorise'])) ? 'checked' : '' ?>>
